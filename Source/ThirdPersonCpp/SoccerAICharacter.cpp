...
void ASoccerAICharacter::ApplyPlayerProfilePhysicalTuning()
{
	...
	if (HasPlayerProfile())
	{
		UE_LOG(
			LogTemp,
			Display,
			TEXT("[PlayerProfile] AI %s applied: Pace=%d Acceleration=%d Stamina=%d Recovery=%d Agility=%d Balance=%d BallControl=%d Dribbling=%d Pass=%d ShotAcc=%d ShotPower=%d Composure=%d OffBall=%d Decision=%d DefReaction=%d Anticipation=%d DefPosition=%d Marking=%d Tackling=%d, FastRun=%.1f, MaxAcceleration=%.1f."),

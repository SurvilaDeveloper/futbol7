...
void AThirdPersonCppCharacter::ApplyPlayerProfilePhysicalTuning()
{
	...
	if (HasPlayerProfile())
	{
		UE_LOG(
			LogTemp,
			Display,
			TEXT("[PlayerProfile] Human %s applied: Pace=%d Acceleration=%d Stamina=%d Recovery=%d Strength=%d Agility=%d Balance=%d BallControl=%d Dribbling=%d Pass=%d ShotAcc=%d ShotPower=%d Tackling=%d Aerial=%d Composure=%d OffBall=%d Decision=%d DefReaction=%d Anticipation=%d DefPosition=%d Marking=%d GKReflex=%d GKPosition=%d GKHandling=%d GKDiving=%d GKDistribution=%d, FastRun=%.1f, MaxAcceleration=%.1f."),
			*GetPlayerProfileId().ToString(),
			...
			GetPlayerProfile()->Attributes.Physical.Strength,
			...
			GetPlayerProfile()->Attributes.Technical.AerialAbility,
			GetPlayerProfile()->Attributes.Tactical.Composure,
			GetPlayerProfile()->Attributes.Tactical.OffBallPositioning,
			GetPlayerProfile()->Attributes.Tactical.DecisionMaking,
